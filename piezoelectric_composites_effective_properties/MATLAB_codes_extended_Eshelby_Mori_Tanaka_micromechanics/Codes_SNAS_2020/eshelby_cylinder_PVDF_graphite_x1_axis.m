%% calculating the electroelastic properties of PVDF/Graphite composite using 
...
% this is cylinder problem when the axis of orientation is about the x1
%axis
...
%          0, 0, 0, 0, e15_f, 0, -k1_f./e0, 0, 0;
%          0, 0, 0, e24_f, 0, 0, 0, -k2_f./e0, 0;
%          e31_f, e32_f, e33_f, 0, 0, 0, 0, 0,-k3_f./e0];

%% material properties of graphite fiber
C11_f=243.7e9;% units in GPa,Component of stiffness matrix
C12_f=6.7e9;% units in GPa,Component of stiffness matrix
C13_f=6.7e9;% units in GPa,Component of stiffness matrix
C22_f=24e9;% units in GPa,Component of stiffness matrix
C23_f=9.7e9;% units in GPa,Component of stiffness matrix
C33_f=24e9;% units in GPa,Component of stiffness matrix
C44_f=11e9;% units in GPa,Component of stiffness matrix
C55_f=27e9;% unitss in GPa,Component of stiffness matrix
C66_f=27e9;% units in GPa,Component of stiffness matrix
k1_f=12*e0;% Component of dielectric modulus matrix,Units m.^(-3).kg.^(-1).sec.^4.Amp.^2
k2_f=12*e0;% Component of dielectric modulus matrix,Units m.^(-3).kg.^(-1).sec.^4.Amp.^2
k3_f=12*e0;% Component of dielectric modulus matrix,Units m.^(-3).kg.^(-1).sec.^4.Amp.^2
e15_f=0;%units [C./m.^2], Component of coupling or piezolectric modulus matrix (in stress-charge form)
e24_f=0;%units [C./m.^2], Component of coupling or piezolectric modulus matrix (in stress-charge form)
e31_f=0;%units [C./m.^2], Component of coupling or piezolectric modulus matrix (in stress-charge form)
e32_f=0;%units [C./m.^2], Component of coupling or piezolectric modulus matrix (in stress-charge form)
e33_f=0;%units [C./m.^2], Component of coupling or piezolectric modulus matrix (in stress-charge form)
E_f=     [C11_f,C12_f, C13_f, 0, 0, 0, 0, 0, e31_f;
         C12_f,C22_f, C23_f, 0, 0, 0, 0, 0, e32_f;
         C13_f,C23_f, C33_f, 0, 0, 0, 0, 0, e33_f;
...
%%
fileID = fopen('cylinder_pvdf_graphite_x1_axis.xls','w');
fprintf(fileID,'%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\n', 'vf', 'E1[GPa]', 'E2[GPa]', 'E3[GPa]', 'G23[GPa]', 'G13[GPa]', 'G12[GPa]', 'nu23', 'nu13', 'nu12', 'd31[C/N]', 'd32[C/N]', 'd33[C/N]', 'd15[C/N]', 'd24[C/N]', 'k11','k22','k33');
fileid2=fopen('cylinder_PVDF_graphite_x1_axis_extra_properties.xls','w')
...
E_composite(:,:,i)=E_m+vf*((E_f-E_m)*A_N*inv((1-vf)*I+vf*A_N));
 E_composite_compliance = inv(E_composite(:,:,i));
...
xlabel('Volume Fraction of Graphite')
ylabel('Youngs Moduli')
...
xlabel('Volume Fraction of Graphite')
ylabel('Piezoelectric Strain Constants')
